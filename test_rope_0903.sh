...
do
    ...
    do
        ...
        do
            # 执行命令
            python attn_rope_std.py -batch_size "$batch_size" -head_dim "$head_dim" -seq_len "$seq_len"
            python attn_rope_flash.py -batch_size "$batch_size" -head_dim "$head_dim" -seq_len "$seq_len"
        done
    done
done
...
do
    ...
    do
        for seq_len in "${seq_lens[@]}"
        do
            python attn_rope_cinn.py -batch_size "$batch_size" -head_dim "$head_dim" -seq_len "$seq_len"
        done
    done
done