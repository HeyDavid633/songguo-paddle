# 9.03 attn_rpoe_std 
# 
# 在这里尝试加上 ROPE 变化使得attntion子图变得更加复杂
#
# python attn_rope_std.py
# python attn_rope_std.py -batch_size 16 -head_dim 32 -seq_len 128  
import timeit
import argparse 
import config
import paddle
...
def attention_RoPE(query, key, value, use_RoPE=True):
...
if __name__ == '__main__':
    ...
    for _ in range(warmup_times + running_times):
        ...
        out = attention_RoPE(query, key, value, use_RoPE=True)
        
    paddle.device.synchronize()
    end_time = timeit.default_timer()

    print("RoPE Std Attn | {:2d} | {:2d} | {:4d} |  Time costs: {:.3f} ms / time".format(batch_size, head_dim, seq_len, (end_time - start_time) * 1000 / running_times)) 
        
    attn_standard_filename = 'attn_rope_std_time.txt'
    with open(attn_standard_filename, 'a') as f: 
        print("ROPE Std Attn | {:2d} | {:2d} | {:4d} |  Time costs: {:.3f} ms / time".format(batch_size, head_dim, seq_len, (end_time - start_time) * 1000 / running_times), file=f) 
