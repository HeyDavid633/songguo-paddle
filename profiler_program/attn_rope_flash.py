# 9.03 attn_rope_flash
#
# python attn_rope_flash.py
# python attn_rope_flash.py -batch_size 16 -head_dim 32 -seq_len 128  
import timeit
import argparse 
import config
import paddle
...
if __name__ == '__main__':
    ...
    print("RoPE Flash Attn | {:2d} | {:2d} | {:4d} |  Time costs: {:.3f} ms / time".format(batch_size, head_dim, seq_len, (end_time - start_time) * 1000 / running_times)) 
    
    attn_flash_filename = 'attn_rope_flash_time.txt'
    with open(attn_flash_filename, 'a') as f: 
        print("RoPE Flash Attn | {:2d} | {:2d} | {:4d} |  Time costs: {:.3f} ms / time".format(batch_size, head_dim, seq_len, (end_time - start_time) * 1000 / running_times), file=f) 
