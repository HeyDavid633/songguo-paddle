...
def check_equal(tensor_a, tensor_b, tolerance=0.1):  
    """  
    比较两个张量的元素是否相等，考虑到精度误差。  
      
    :param tensor_a: 第一个张量  
    :param tensor_b: 第二个张量  
    :param tolerance: 允许的最大误差，默认为0.000001  
    :return: 相等比率（百分比）  
    """  
    # 确保两个张量的形状相同  
...
if __name__ == "__main__":  
    tensor_a = paddle.to_tensor([1.00001, 3.002, 3.03], dtype='float32')  
    tensor_b = paddle.to_tensor([1.0, 2.0, 3.0], dtype='float32')  
    check_equal(tensor_a, tensor_b)